...
class Mov_Next_RegIO extends Bundle {
  // 从MovUnit接收结果
  val in = Flipped(ValidIO(new MovResultEntry))       // MovUnit执行结果

  // 输出到下一级
  val out = Output(new MovResultEntry)                // 直接输出结果
...
class Mov_Next_Reg extends Module {
  ...
  // 寄存器状态
  val reg = RegInit(0.U.asTypeOf(new MovResultEntry))
  ...
  when(internal_flush) {
    ...
    valid := false.B
    reg := 0.U.asTypeOf(new MovResultEntry)
